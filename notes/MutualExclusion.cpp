...
2. Lamport 细节：
    a. 每条消息都使用其发送者的当前逻辑时间作为时间戳；
    b. 多播消息也都被发送到消息的发送者（意思是发送者自己也会收到自己发送的消息）；
    c. 假设一个发送者发送的所有消息都按照它们发送的顺序被接收，并且没有消息丢失；
    d. 进程把接收到的消息放入一个本地的队列中，按照消息的时间戳顺序；
    e. 消息的接受者向其他所有进程多播一个ACK（确认消息收到）；
    f. 只有在队列头部和所有参与者都认可的情况下才能发送消息？？ （什么叫队列头部认可？）；
...
2. 当一个进程想要访问C.S.时，它就发送一个带有时间戳的请求到其他各个进程（包括自己）：
    a. 等待其他进程的响应；
    b. 如果自己的请求位于其队列的头部并且已收到所有答复，进入C.S.；
    c. 退出C.S.时，从队列中删除它的请求，并向每个进程发送释放消息。

3. 对于其他进程：
    a. 收到一个请求后，在自己的请求队列（按时间戳排序）中输入请求并回复一个时间戳，这个回复是单播的，只用响应消息的发送者；
    b. 收到释放消息后，从自己的请求队列中删除相应的请求；
    c. 如果自己的请求位于队列的头部，并且收到了全部其他进程的响应，就进入C.S.
...
3. R m 表示 Receive message m；
4. B m 表示 Broadcast message m to all other processes；
5. S m to j表示 Send message m to process j；
