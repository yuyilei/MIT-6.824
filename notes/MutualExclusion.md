...
所以，Li(a).i 发生在Lj(b).j 之前表示为：Li(a) < Lj(b) 或 Li(a) = Lj(b)，i < j
...
3. 对于其他进程：
    * 收到一个请求后，在自己的请求队列（按时间戳排序）加入请求，并向所有进程返回一个ACK；
    * 收到释放消息后，从自己的请求队列中删除相应的请求；
    * 如果自己的请求位于队列的头部，并且收到了全部其他进程的响应，就进入C.S.

4. 正确性：进程x发送带有时间戳Tx的请求时，它的队列中的请求的时间戳都小于Tx的。

一个Lamport & Totally-Ordered Multicasting的例子：

![](https://upload-images.jianshu.io/upload_images/5753761-fec5c7e077298518.png?imageMogr2/auto-orient/)

旧金山客户增加100美元，纽约银行增加1％的利息。

不同的操作顺序导致不同的副本的不一致性：旧金山将有1,111美元，纽约将有1,110美元。可以使用Lamport的完全有序。

操作：
1. 发送消息时带上发送者的时间戳（逻辑时钟+id）；
2. 发送消息是多播的（包括发给自己）；
3. 收到一个消失时：
    * 放入本地队列
    * 本地队列根据时间戳排序
...
只有满足以下条件时，消息才会被发送到应用程序（被执行）：
1. 消息在队列的头部；
2. 发起的消息的进程收到了全部进程的ACK；
3. 只有满足以下条件进程Pi才会向Pj发ACK：
    * Pi自身没有对同一个共享资源的操作的需求（请求）；
    * Pi的进程号大于Pj的进程号；
    * Pi对同一个共享资源的操作已经完成；
    
将“Add $100”记为消息 m，将“Add interest of 1%”记为消息n。

![](https://upload-images.jianshu.io/upload_images/4440914-87e43ea9d626a95b.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

1. 此时，队列中：
P1: (m,1.1), (n,1.2)；
P2: (m,1.1), (n,1.2)；
2. P1不会返回一个ACK给(n,1.2)，但是会返回一个ACK给(m,1.1)，因为P1也想要访问同一共享资源，且P1的进程号小于P2；（返回ACK给某个消息指的是多播ACK给全部进程，下面也是）
...
![](https://upload-images.jianshu.io/upload_images/4440914-6210be0ca5375069.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)


6. P1完成(m,1.1)之后，它返回ACK给(n,1.2)；
7. P1和P2获得了全部进程对(n,1.2)的ACK，且在队列头部，(n,1.2)被执行；
8. 最终，(m,1.1)和(n,1.2)在全部节点中都以相同顺序被完成，保证了一致性。


![](https://upload-images.jianshu.io/upload_images/4440914-a6112f43330a25ab.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
